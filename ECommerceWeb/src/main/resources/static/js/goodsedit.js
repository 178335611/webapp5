document.getElementById('goodsedit-form').addEventListener
    ...
    fetch('/global')
        ...
        .then(data => {
            ...
            const goodsData = { // 构建要发送的数据对象
                ...
                id: formData.get('goodid') // 使用表单中隐藏的 ID

            };
            // 发送表单数据到后端
            fetch(`/seller/warehouse/goodsedit/${formData.get('goodid')}`, { // 假设后端接收表单数据
                method: 'PUT',
                headers: {
                ...
                .then(responseData => {
                    if (responseData) {
                        alert('商品修改成功');
                        window.location.href = `/seller/warehouse`; // 重定向到商品列表页面
                    } else {
                        // 如果服务器返回了失败信息，显示给用户
                        alert('修改商品失败: ' + ( '未知错误'));
                    }
